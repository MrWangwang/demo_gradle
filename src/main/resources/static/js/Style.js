...
Effect.scroll = function (element, speed, callback) {//窗口滚动到指定元素处：speed:速度 1-100
    ...
    getSetInterval(getElement(effect), "scroll", speed / 100, start, end, ["x", "y"], "setScroll", callback ? function () {
...
Effect.ie6ScrollFix = function () {
    if (isIE6 && !isQuirks) {
        ...
        if ((window.frameElement && sw < window.frameElement.offsetWidth + 30) || (!window.frameElement && sw < document.documentElement.clientWidth + 30)) {
            ...
            if (!Effect.scrollFixOnresize) {
                if (window.frameElement)
                    window.attachEvent('onresize', Effect.ie6ScrollFix);//4
        ...
        if ((window.frameElement && sh > window.frameElement.offsetHeight) || (!window.frameElement && sh > document.documentElement.clientHeight)) {//解决ie6下横向滚动条随竖向滚动条一起出现的问题
