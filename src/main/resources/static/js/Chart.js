...
infosoftglobal.FusionCharts.prototype = {
    ...
    getSWFHTML: function () {
        ...
        if (navigator.plugins && navigator.mimeTypes && navigator.mimeTypes.length) {
            ...
            swfNode += ' id="' + this.getAttribute('id') + '" wmode="opaque" name="' + this.getAttribute('id') + '" ';
...
//this part is added by zving software.inc
var Chart = function (type, id, width, height, method, title) {
    this.Type = type;
    this.ID = id;
    this.Width = width;
    this.Height = height;
...
Chart.prototype.addParam = function (key, value) {
    this.DC.add(key, value);
}

Chart.prototype.show = function (divID) {
    var This = this;
    this.DivID = divID;
    Server.sendRequest(this.Method, this.DC, function (response) {
        This.XML = response.get(0);
        This.render();
    });
}
...
Chart.prototype.render = function () {
    ...
    if (this.Type == "Pie3D") {
        file = "FCF_Doughnut2D";
    } else if (this.Type == "Mixed2D") {
        file = "FCF_MSColumn2DLineDY";
    }
    var chart = new FusionCharts(Server.ContextPath + "Framework/Charts/" + file + ".swf", this.ID, this.Width, this.Height, "0", "0");
    chart.setDataXML(this.XML);
    chart.render(this.DivID);
}