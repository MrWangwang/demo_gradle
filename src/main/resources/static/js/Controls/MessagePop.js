...
MsgPop.prototype.show = function () {
    ...
    if (!boxdiv) {
        ...
        boxdiv.style.cssText = "position:fixed; right:0; bottom:0; width:500px; overflow:visible; z-index:980;";
        if (isIE6) {
...
MsgPop.close = function (id) {
    var pw = $E.getTopLevelWindow();
    pw.eval('window._OpacityPopFunc = function(){pw.$("' + id + '").outerHTML = "";pw.MsgPop._Array.remove("' + id + '");}');
    pw.Effect.fade(pw.$(id), 3, 0, 0.5, pw._OpacityPopFunc);
}

MsgPop.closeSelf = function () {//在MsgPop内的HTML中可以调用MsgPop.closeSelf方法来关闭自己所在的MsgPop
    if (window != window.parent) {
        var boxId = window.frameElement.getAttribute("boxId");
        if (boxId != null && boxId != "") {
            parent.MsgPop.close(boxId);
        }
    } else {//没有iframe
        if (MsgPop._Array.length > 0) {
            var id = MsgPop._Array[MsgPop._Array.length - 1];
            MsgPop.close(id);
        }
    }
}