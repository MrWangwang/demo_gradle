...
Selector.show = function (ele) {
    ...
    if (ele.id != pw.Selector.showingID || !pw.Selector.showingID) {
        ...
        if (url) {
            ...
            f._OnLoad = function () {
                Selector.removeFrameMouseDownEvent(f.contentWindow);
            ...
            if (!pw.$("_SelectorFrame")) {
                ...
                doc.write(".optgroup a:hover,.optgroup a.ahover{color: #cff;text-decoration:none;background:#49e url(" + Server.ContextPath + "js/Images/optionbg_over.gif) repeat-x center;}");
...
Selector.getSourceDiv = function (id) {//要兼顾初次加载的情况
    var div = null;
    var pw = $E.getTopLevelWindow();
    if (pw.SelectorWindow && pw.SelectorWindow.$) {
        div = pw.SelectorWindow.$(id);
    }
    if (!div) {
        div = $(id);
        if (!div || !div.InitFlag) {
            //在Selector.onItemMouseOver()时pw.SelectorWindow可能已经置为空
...
Selector.setSelectedIndex = function (ele, index) {
    ...
    if (oldValue != item.$A("value")) {
        try {
            ...
            alert("Selector.setSelectedIndex():" + ex.message);
        }
        ;
    }
}
...
Selector.onItemClick = function (ele, flag) {//注意，此方法是从未引用Main.js的iframe中调用
    ...
    if (oldValue != ele.getAttribute("value")) {
        try {
            ...
            alert("Selector.onItemClick():" + ex.message);
        }
        ;
    }
    Selector.close();
...
Selector.close = function (evt) {
    ...
    if (pw.Selector && pw.Selector.showingID) {
        if (pw.SelectorWindow) {
            var ctrl = pw.SelectorWindow.$(pw.Selector.showingID).textField;
            ...
            Misc.unlockScroll(pw.SelectorWindow);
            pw.SelectorWindow = null;
        }
        $E.hide(pw.$("_SelectorDiv_"));
        //pw.SelectorWindow = null;//有可能导致找不到DIV
    ...
    if (!flag || flag == "false") {
        if (isIE) {
            ...
            ele.textField.onmousedown = function (evt) {
                ...
                if (div.id == pw.Selector.showingID && pw.SelectorWindow == window) {
...
Selector.invokeOnChange = function (_ele) {
    if (!_ele.InitFlag) {
        ...
        if (_cv) {
            var pw = $E.getTopLevelWindow();
            try {
                if (pw.SelectorWindow && pw.SelectorWindow != window) {
                    pw.SelectorWindow.eval("window.func = function(){" + _cv + "}");
                    pw.SelectorWindow.func.call(_ele);
                } else {
...
Selector.initHtml = function (ele) {
    ...
    if (len > 0 || ele.innerHTML.trim() == "") {
        ...
        arr.push("<img class='arrowimg' src='" + Server.ContextPath + "js/Images/blank.gif' width='18' height='20' id='" + id + "_arrow' style='position:relative; left:-17px; margin-right:-18px; cursor:pointer; width:18px; height:20px;vertical-align:top; '/>");
...
Selector.setReturn = function (t, v, scrollTop) {
    ...
    if (oldValue != v) {
        try {
            ...
            alert("Selector.setReturn():" + ex.message);
        }
        ;
    }
