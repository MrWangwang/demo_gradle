...
Paper.prototype = {
    ...
    attachBehaviors: function () {
        var self = this;
        if (this.drawable) {
            this.elem.attachEvent("onmousedown", function (evt) {
...
Rect.prototype = {
    ...
    start: function (evt) {
        ...
        var self = this;
        this._fMove = function (evt) {
            self.move(evt)
        }
        this._fFinish = function (evt) {
            self.finish(evt)
        }
    ...
    attachBehaviors: function () {
        var self = this;
        if (this.editable) {
            this.elem.attachEvent("ondblclick", function (evt) {
                self.callFunc(self.hashDataEle, evt)
            })
            this.elem.attachEvent("onmousedown", function (evt) {
                self.focus(evt)
            });
            ...
            this.elem.attachEvent("onclick", function (evt) {
                self.callFunc(self.hashDataEle, evt)
            });
            this.elem.attachEvent("onmouseover", function (evt) {
                self.focus(evt), self.tipOn(evt);
                ;
            });
            this.elem.attachEvent("onmousemove", function (evt) {
                self.focus(evt), self.tipMove(evt);
            });
            this.elem.attachEvent("onmouseout", function (evt) {
                self.blur(evt), self.tipOff(evt);
            });
        }
    },
    remove: function (evt) {
    ...
    addTitle: function () {
        ...
        var self = this;
        if (isIE) {
            this.hashDataEle.attachEvent("onpropertychange", function (evt) {
                self.onHashDataChange(evt)
            })
        } else {
            this.hashDataEle.attachEvent("onDOMAttrModified", function (evt) {
                self.onHashDataChange(evt)
            })
        }
...
Drag.prototype = {
    ...
    init: function (drag, options) {
        ...
        //修正范围
        this.Repair();
        this._Handle.attachEvent("onmousedown", function (evt) {
            self.Start(evt)
        })
    },
    //设置默认属性
