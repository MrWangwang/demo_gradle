    ...
    for (var i = 0; i < portalData.length; i++) {
        if (Portal.isInner(curPos, portalData[i].element.PosAndDim)) {//光标在某一列内
            ...
            if (!feed) {//在列内，却不在列内的某个portalet上
                ...
                if (feed.element == this) {
                    dostIndex--;
                    feed = curColFeeds[dostIndex];
                }//正在拖拽的portalet不再用来检测
                if (!feed)return;
...
Portal.Init = function () {//把页面上的portalet与portalData相关联
    ...
    for (var i = 0; i < portalData.length; i++) {
        ...
        if (!colEle) {
            alert("当前页面找不到配置信息中所提供的id为 " + portalData[i].id + " 的页面元素");
            return;
        }
        ...
        for (var j = 0; j < portalData[i].feeds.length; j++) {
            ...
            if (!modEle) {
                alert("当前页面找不到配置信息中所提供的 #" + portalData[i].id + " 下id为 " + portalData[i].feeds[j].id + " 的页面元素");
                return;
            }
...
Portal.addPortlet = function (feedObj) {
    ...
    if (id == undefined || widgetType == undefined) {
        Dialog.alert("错误的参数");
        return;
    }
    if ($(id)) {
        Dialog.alert("已经存在名为" + title + "的模块了!");
        return;
    }
    ...
    Server.sendRequest("com.zving.portal.Portal.getPortletHtml", dc, function (response) {
        ...
        if (!modEle) {
            alert("当前页面找不到配置信息中所提供的 #" + portalData[0].id + " 下id为 " + portalData[0].feeds[0].id + " 的页面元素");
            return;
        }
