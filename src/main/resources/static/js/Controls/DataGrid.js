...
DataGrid.onSort = function (ele) {
    ...
    if (table.id.endsWith("_HeadTable")) {
        table = $(table.id.substring(0, table.id.lastIndexOf("_")));
    }
...
DataGrid.init = function (ele) {
    ...
    ele.loadData = function (func, dc) {
        DataGrid.loadData(id, func, dc);
    }
    ele._insertRow = ele.insertRow;
    ele._deleteRow = ele.deleteRow;
    ele.insertRow = function (index, editFlag, func) {
        DataGrid.insertRow(id, index, editFlag, func);
    }
    ele.deleteRow = function (index) {
        DataGrid.deleteRow(id, index);
    }
    ele.edit = function (evt) {
        DataGrid.edit(evt, id);
    }
    ele.filter = function (func) {
        DataGrid.filter(id, func);
    }
    ele.getParam = function (name) {
        return DataGrid.getParam(id, name);
    }
    ele.setParam = function (name, value) {
        DataGrid.setParam(id, name, value);
    }
    ele.toExcel = function (toExcelpageFlag) {
        DataGrid.toExcel(id, toExcelpageFlag);
    }
    ele.allToExcel = function () {
        DataGrid.allToExcel(id);
    }
    ele.save = function (method, func) {
        DataGrid.save(id, method, func);
    }
    ele.select = function (v) {
        DataGrid.select(id, v);
    }
    ele.sort = function (field, order) {
        DataGrid.sort(id, field, order);
    }

    ele.showLoading = function () {
        DataGrid.showLoading(id);
    }
    ele.selectAll = function () {
        DataGrid.selectAll(id);
    }
    ele.unselectAll = function () {
        DataGrid.unselectAll(id);
    }
    ele.getSelectedData = function () {
        return DataGrid.getSelectedData(id);
    }
    ele.getSelectedRows = function () {
        return DataGrid.getSelectedRows(id);
    }
    ele.getSelectedTreeValue = function () {
        return DataGrid.getSelectedTreeValue(id);
    }
    ele.getSelectedValue = function () {
        return DataGrid.getSelectedValue(id);
    }
    ele.getSelectors = function () {
        return DataGrid.getSelectors(id);
    }
    ele.getColumnInfo = function () {
        return DataGrid.getColumnInfo(id);
    }
    ele.selectedRowToExcel = function () {
        DataGrid.selectedRowToExcel(id);
    }
    ele.setFixedHeight = function (h) {
        DataGrid.setFixedHeight(id, h);
    }
    ele.setFixedWidth = function (w) {
        DataGrid.setFixedWidth(id, w);
    }

    ele.getPageSize = function () {
        return DataGrid.getParam(id, Constant.PageSize);
    }
    ele.getPageIndex = function () {
        return DataGrid.getParam(id, Constant.PageIndex);
    }
    ele.setPageSize = function (size) {
        DataGrid.setParam(id, Constant.PageSize, size);
    }
    ele.setPageIndex = function (index) {
        DataGrid.setParam(id, Constant.PageIndex, index);
    }

    ele.isPageFlag = function () {
        return $(id).$A("page") == "true";
    }
    ele.isLazy = function () {
        return $(id).$A("lazy") == "true"
    }
    ele.isScroll = function () {
        return $(id).$A("scroll") == "true"
    }
    ...
    if (page && ele.rows.length <= 3) {
        ...
        ele.RowHeight = $E.getDimensions(ele.rows[1]).height;
    } else {
        ele.RowHeight = 23;
    }
    if (!ele.RowHeight) {
...
DataGrid.getColumnInfo = function (ele) {
    ...
    for (var i = 0; i < headTR.cells.length; i++) {
        ...
        if (td.innerText) {//没有标题的列不要
            var w = isIE ? td.currentStyle.width : td.offsetWidth;//$E.getDimensions()在单元格宽度为百分比值时有误
            w = w + ""
            if (w.indexOf("%") > 0) {
                w = $(ele).getDimensions().width * parseInt(w.replace(/\D/gi, '')) / 100;
...
DataGrid.toExcelSubmit = function (ele, toExcelPageFlag, columnIndexes, columnWidths, rows) {
    ...
    if (rows) {
        input = $(xls + "_ZVING_Rows");
        if (!input) {
            input = document.createElement("input");
        }
        input.type = "hidden";
        input.id = xls + "_ZVING_Rows";
        input.name = xls + "_ZVING_Rows";
...
DataGrid.showLoading = function (ele) {
    ele = $(ele);
    var bgdiv = $("_LoadingBGDiv");
    var icondiv = $("_LoadingIconDiv");
    if (!bgdiv) {
        ...
        $E.hide(icondiv);
        icondiv.innerHTML = "　<img src='" + Server.ContextPath + "js/Images/loadingGreen15px.gif'><font color=green> 正在加载......　</font>";
...
DataGrid.loadData = function (ele, func, dc) {
    ...
    try {
        ...
        var selectedValue = ele.getSelectedValue();
        if (selectedValue) {
            window.currentSelected = selectedValue;
        }
    ...
    Server.sendRequest("com.zving.framework.controls.DataGridPage.doWork", dc, function (response) {
        try {
            ...
            var newEle = null;
            var table = ele.tagName.toLowerCase() == "table" ? ele : $(id);
            ...
            if (DataGrid.getParam(id, "_ZVING_SCROLL") == "true") {
            ...
            var scripts = table.$T("script");
            for (var i = 0; i < scripts.length; i++) {
                eval(scripts[i].text);
            }
            eval("DataGrid_" + id + "_Init();");
            if (window.currentSelected) {
                for (var i = 0; i < window.currentSelected.length; i++) {
                    DataGrid.select(table, window.currentSelected[i]);
                }
            }
            if (func) {
                try {
...
DataGrid.clear = function (ele) {
    ...
    if (!ele.isPageFlag()) {
        ...
        if ($(ele.id + "_PageBar")) {
            $(ele.id + "_PageBar").innerHTML = "<div style='float:right;font-family:Tahoma'>第一页&nbsp;|&nbsp;上一页&nbsp;|&nbsp;下一页&nbsp;|&nbsp;最末页&nbsp;&nbsp;转到第&nbsp;<input id='_PageBar_Index' type='text' class='inputText' style='width:40px'>&nbsp;页&nbsp;&nbsp;<input type='button' class='inputButton' value='跳转'></div><div style='float:left;font-family:Tahoma'>共 0 条记录，每页 10 条，当前第 0 / 0 页</div>";
        }
    }
    DataGrid.autoFillBlank(ele);
    ele.DataSource.Rows = [];
...
DataGrid.autoFillBlank = function (ele) {//自动用空白行撑开DataGrid
    ...
    if (ele.$A("autoFill") != "false") {
        ...
        //alert(ele.id+".RowHeight:"+ele.RowHeight);
...
DataGrid.treeClick = function (ele) {
    ...
    if (ele.src.indexOf("butExpand") > 0) {
        ele.src = "" + Server.ContextPath + "js/Images/butCollapse.gif";
        hideFlag = true;
    } else {
        ele.src = "" + Server.ContextPath + "js/Images/butExpand.gif";
    }
    var rows = table.rows;
...
DataGrid.insertRow = function (ele, index, editFlag, func) {
    ...
    try {
        ...
        Server.sendRequest("com.zving.framework.controls.DataGridPage.doWork", dc, function (response) {
            ...
            if (editFlag) {
                DataGrid.changeStatus($(id).rows[index + 1]);
            } else {
...
DataGrid.changeStatus = function (ele, type) {//改变编辑状态
    ...
    for (var i = 0; i < headTR.cells.length; i++) {
        ...
        if (ztype && ztype.toLowerCase() == "selector") {
            $(dg.id + "_RowCheck" + ele.rowIndex).disabled = ele.EditStatus ? true : false;
            ...
            $(dg.id + "_" + field + "_Checkbox" + ele.rowIndex).disabled = ele.EditStatus ? false : true;
        } else if (ztype && ztype.toLowerCase() == "dropdownlist") {
            if (v1) {
                modifyFlag = v1[ds.ColMap[field.toLowerCase()]] != dr.get(field);
            }
            $(dg.id + "_" + field + "_DropDownList" + ele.rowIndex).disabled = ele.EditStatus ? false : true;
        } else {
            var arr = [];
            if (ztype && ztype.toLowerCase() == "tree") {
                ...
                if (level < nextLevel) {
                    if (nextVisiable) {
                        arr.push("<img src='" + Server.ContextPath + "js/Images/butExpand.gif' onclick='DataGrid.treeClick(this)'/>&nbsp;");
                    } else {
                        arr.push("<img src='" + Server.ContextPath + "js/Images/butCollapse.gif' onclick='DataGrid.treeClick(this)'/>&nbsp;");
                    }
                } else {
                    arr.push("<img src='" + Server.ContextPath + "js/Images/butNoChild.gif'/>&nbsp;");
                }
            }
...
DataGrid.setFixedHeight = function (ele, h) {
    ...
    $(ele.id + "_Wrap_body").style.height = h;
...
DataGrid.initScroll = function (ele) {
    ele = $(ele);
    var id = ele.id;
    ele.Wrapper = $(id + "_Wrap");
    ele.Wrapper.head = $(id + "_Wrap_head");
    ele.Wrapper.head.table = ele.Wrapper.head.children[0];
    ele.Wrapper.body = $(id + "_Wrap_body");
...
DataGrid.resetWidth = function (ele) {
    ...
    if (ele.Wrapper.scrollWidth > ele.Wrapper.body.table.offsetWidth) {
        var bodyTHs = ele.Wrapper.body.table.rows[0].cells;
        headTHs = ele.Wrapper.head.table.rows[0].cells;
